            ...
            {cartItems.map((item, idx) => (
              ...
              >
                ...
                <div className="w-32 h-32 bg-white rounded-xl flex flex-shrink-0 items-center justify-center overflow-hidden border border-gray-100 p-2">
                   {item.image ? (
                     <img src={item.image} alt={item.seoAlt || `${item.name} - Agrishield India Crop Protection`} title={item.seoTitle || `${item.name} | Agrishield Farm Store`} width="128" height="128" loading="lazy" decoding="async" className="w-full h-full object-contain" />
                   ) : (
